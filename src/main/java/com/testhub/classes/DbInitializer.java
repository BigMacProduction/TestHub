...
public class DbInitializer {
	...
	static String createRoleTableStatement = " CREATE TABLE `" + Container.getSqlSchema() + "`.`role` "
			+ "( `roleId` int(11) NOT NULL, " + "`name` varchar(50) NOT NULL, "
	...
	static String createAccountTableStatement = "CREATE TABLE `" + Container.getSqlSchema() + "`.`account`"
			+ " ( `accountId` INT NOT NULL AUTO_INCREMENT, " + "`firstName` VARCHAR(50) NOT NULL, "
			+ "`lastName`  VARCHAR(50) NOT NULL, " + "`login`     VARCHAR(50) NOT NULL, "
			+ "`email`     VARCHAR(255) NOT NULL, " + "`password`  VARCHAR(255) NOT NULL, "
			+ "`createDate`DATETIME NULL, " + " PRIMARY KEY (`accountId`));";

	static String createRawAccountTableStatement = "CREATE TABLE `" + Container.getSqlSchema() + "`.`raw_account`"
			+ " ( `accountId` INT NOT NULL AUTO_INCREMENT, " + "`firstName` VARCHAR(50) NOT NULL, "
			+ "`lastName`  VARCHAR(50) NOT NULL, " + "`login`     VARCHAR(50) NOT NULL, "
			+ "`email`     VARCHAR(255) NOT NULL, " + "`password`  VARCHAR(255) NOT NULL, "
			+ "`validationExpression` VARCHAR(40) NOT NULL, " + "`createDate`DATETIME NULL, "
			+ " PRIMARY KEY (`accountId`));";

	static String createAccountRoleStatement = "CREATE TABLE `" + Container.getSqlSchema() + "`.`account_role` "
			+ "( `accountId` int(11) NOT NULL, " + "`roleId` int(11) NOT NULL ) DEFAULT CHARSET=utf8";

	static String createTestTableStatement = "CREATE TABLE `" + Container.getSqlSchema() + "`.`test` "
			+ "( `testId`     INT NOT NULL, " + "  `name`       VARCHAR(255) NOT NULL, "
			+ "  `author`     INT NOT NULL, " + "  `createDate` TIMESTAMP(6) NOT NULL, "
			+ "  `testcol`    VARCHAR(45) NOT NULL, " + "PRIMARY KEY (`testId`));";

	static String createHistoryTableStatement = " CREATE TABLE `" + Container.getSqlSchema() + "`.`test_history` "
			+ "( `accountId` INT NOT NULL, " + "`passingDate` DATETIME NULL, " + "`testId`      INT NOT NULL, "
			+ "`score`       INT NOT NULL);";

	static String createPasswordRecoveryTableStatement = "CREATE TABLE `" + Container.getSqlSchema()
			+ "`.`passwordRecovery` "
			+ "(`email` VARCHAR(45) NOT NULL,`recoveryExpression` VARCHAR(45) NULL, PRIMARY KEY (`email`))";

	//
	// CREATE TABLE `testhub`.`passwordRecovery` (
	// `email` VARCHAR(45) NOT NULL,
	// `recoveryExpression` VARCHAR(45) NULL,
	// PRIMARY KEY (`email`));
	...
	public void generateDb() {
		...
		System.out.println("Trying to do CREATE raw_account TABLE");

		executeStatement(createRawAccountTableStatement);
		...
		executeStatement(createPasswordRecoveryTableStatement);
	}

}
/*
 * CREATE TABLE `firsttest` ( `question` varchar(255) NOT NULL, `var1`
 * varchar(45) NOT NULL, `var1ans` tinyint(1) NOT NULL, `var2` varchar(45) NOT
 * NULL, `var2ans` tinyint(1) NOT NULL, `var3` varchar(45) DEFAULT NULL,
 * `var3ans` tinyint(1) DEFAULT NULL, `var4` varchar(45) DEFAULT NULL, `var4ans`
 * tinyint(1) DEFAULT NULL, PRIMARY KEY (`question`) ) ENGINE=InnoDB DEFAULT
 * CHARSET=utf8
 */